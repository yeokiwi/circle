...
// The on-board Ethernet controller is used for EtherCAT, exclusively on
// core 1 (raw SOEM cyclic loop, no CScheduler). The second Ethernet port
// (normally a USB Ethernet device, "eth1") runs the TCP/IP stack with the
// ZeroMQ client, on core 0 (CScheduler-based, like all networking in Circle).
// Swap these values, if you want it the other way around.
...
/// \brief Runs the EtherCAT master on core 1. Cores 2/3 are unused.
class CEtherCATCoreRunner : public CMultiCoreSupport
{
public:
	CEtherCATCoreRunner (CMemorySystem *pMemorySystem, CEtherCATCore1Master *pMaster)
...
private:
	CEtherCATCore1Master *m_pMaster;
};
...
private:
	...
	CEtherCATCoreRunner    *m_pCoreRunner;		// starts core 1, see above
