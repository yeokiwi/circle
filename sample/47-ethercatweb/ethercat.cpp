...
// The EtherCAT master task needs a bigger stack than the default, because
// SOEM uses relatively large buffers on the stack.
#define ETHERCAT_TASK_STACK_SIZE	(64 * 1024)
...
CEtherCATMaster::CEtherCATMaster (unsigned nDeviceIndex, unsigned nCycleTimeUs)
:	CTask (ETHERCAT_TASK_STACK_SIZE),
	...
	m_pStatus (0)
{
	...
	// on the heap, because they are too big for the stack of a task.
...
CEtherCATMaster::~CEtherCATMaster (void)
{
	if (m_bInitialized)
	{
...
boolean CEtherCATMaster::Configure (void)
{
	...
	// have to be updated according to the PHY status by this task.
...
void CEtherCATMaster::CyclicProcess (void)
{
...
boolean CEtherCATMaster::CheckSlaveStates (void)
{
	assert (m_pContext != 0);
	...
	// one or more slaves are not (yet) operational, check them one by one
	pGroup->docheckstate = FALSE;

	ecx_readstate (m_pContext);

	boolean bAllOperational = TRUE;
	...
	for (int nSlave = 1; nSlave <= m_pContext->slavecount; nSlave++)
	{
		...
		else if (pSlave->state == EC_STATE_SAFE_OP)
		{
			CLogger::Get ()->Write (FromEtherCAT, LogWarning,
		...
		{
			ecx_statecheck (m_pContext, nSlave, EC_STATE_OPERATIONAL, EC_TIMEOUTRET);
...
void CEtherCATMaster::ReadSlaveInfo (void)
{
	assert (m_pContext != 0);
	assert (m_pSlaveInfo != 0);
...
void CEtherCATMaster::ReadSlaveSDOString (unsigned nSlave, u16 usIndex,
...
void CEtherCATMaster::UpdateStatus (void)
{
	assert (m_pStatus != 0);
	assert (m_pContext != 0);
...
void CEtherCATMaster::GetStatus (TEtherCATStatus *pStatus)
{
	assert (pStatus != 0);
	assert (m_pStatus != 0);
...
void CEtherCATMaster::SetError (const char *pMessage)
{
	assert (pMessage != 0);
...
void CEtherCATMaster::ClearError (void)
{
	m_LastError = "";
	m_LoggedError = "";
}

void CEtherCATMaster::SetState (TEtherCATMasterState State)
{
	m_State = State;
}

const char *CEtherCATMaster::GetMasterStateString (TEtherCATMasterState State)
{
...
const char *CEtherCATMaster::GetSlaveStateString (u16 nState)
{
