...
:	m_Screen (m_Options.GetWidth (), m_Options.GetHeight ()),
	...
	m_Net (0, 0, 0, 0, "elmoweb", NetDeviceTypeEthernet, WEBSERVER_NET_DEVICE),
...
TShutdownMode CKernel::Run (void)
{
	...
	m_pElmoMaster = new CElmoMaster (ETHERCAT_NET_DEVICE, ETHERCAT_CYCLE_TIME_US);
	assert (m_pElmoMaster != 0);

	if (m_bNetAvailable)
	{
		new CWebServer (&m_Net, m_pElmoMaster);
	}
...
void CKernel::ShowStatus (void)
{
	...
	TElmoStatus *pStatus = new TElmoStatus;
	if (pStatus == 0)
	{
		return;
	}
	...
	m_Logger.Write (FromKernel, LogNotice,
			"EtherCAT: %s, %u drive(s), WKC %d of %d, %llu cycle(s), "
			"%llu missed deadline(s)",
			pStatus->MasterState, pStatus->nDriveCount, pStatus->nWorkCounter,
			pStatus->nExpectedWorkCounter,
			(unsigned long long) pStatus->nCycles,
			(unsigned long long) pStatus->nMissedDeadlines);
