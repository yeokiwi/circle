...
boolean CKernel::Initialize (void)
{
	...
	if (bOK)
	{
		...
		// The Elmo Platinum master (on core 1) works without it.
		if (CNetDevice::GetNetDevice (NetDeviceTypeEthernet, ZMQ_NET_DEVICE) != 0)
		{
			...
			// drive master can be started, while the link is still down
	...
	if (bOK)
	{
		if (CNetDevice::GetNetDevice (NetDeviceTypeEthernet, ELMO_NET_DEVICE) == 0)
		{
			m_Logger.Write (FromKernel, LogError,
					"Net device %u for the Elmo Platinum drive not found",
					ELMO_NET_DEVICE);
		}

		m_pElmo = new CElmoPlatinumCore1Master (ELMO_NET_DEVICE, ELMO_CYCLE_TIME_US);
		assert (m_pElmo != 0);
		...
		if (m_bNetAvailable)
		{
			...
			m_pZeroMQ = new CZeroMQClient (&m_Net, m_pElmo, ZMQ_STATUS_INTERVAL_MS);
			assert (m_pZeroMQ != 0);
	...
	if (bOK)
	{
		...
		// scheduler) is complete. From this point on, the drive master runs
		// concurrently on core 1. The drive stays disarmed (see
		// elmo_core1.cpp) until a "start" command is received over ZeroMQ.
		m_pCoreRunner = new CElmoCoreRunner (CMemorySystem::Get (), m_pElmo);
		...
		{
			m_Logger.Write (FromKernel, LogError,
					"Cannot start core 1 for the Elmo Platinum master "
...
void CKernel::ShowStatus (void)
{
	...
	m_Logger.Write (FromKernel, LogNotice,
			"EtherCAT: %s, drive: %s, WKC %d of %d, %llu cycle(s), %s",
			CElmoPlatinumCore1Master::GetMasterStateString (pStatus->MasterState),
			CElmoPlatinumCore1Master::GetDriveStateString (pStatus->DriveState),
			pStatus->nWorkCounter, pStatus->nExpectedWorkCounter,
			(unsigned long long) pStatus->nCycles,
			pStatus->bArmed ? "armed" : "disarmed");
