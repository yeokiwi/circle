...
CElmoPlatinumCore1Master::CElmoPlatinumCore1Master (unsigned nDeviceIndex, unsigned nCycleTimeUs)
:	m_nDeviceIndex (nDeviceIndex),
	...
	m_bConfigured (FALSE),
	m_bRunning (FALSE),		// disarmed by default, see README safety section
	m_nExpectedWKC (0),
	m_nWorkCounter (0),
	m_nCycles (0),
	m_nWKCErrors (0),
...
CElmoPlatinumCore1Master::~CElmoPlatinumCore1Master (void)
{
	if (m_bInitialized)
	{
...
void CElmoPlatinumCore1Master::Run (void)
{
...
// 5 ms -> OPERATIONAL. The bus is always brought all the way to OPERATIONAL
// here, regardless of the armed/disarmed intent (m_bRunning): arming is a
// CiA 402 (drive) concept, not an EtherCAT AL-state concept, process data
// has to flow either way, and the drive itself stays safely parked via the
// controlword written every cycle in CyclicProcess() below. Do not restrict
// this to SAFE_OP when disarmed by analogy with
// sample/50-ethercatzmq/ethercat_core1.cpp's Configure() — that class's
// Stop command is a bus-level concept, this class's is not.
//

boolean CElmoPlatinumCore1Master::Configure (void)
{
	if (m_bInitialized)
	{
		ecx_close (m_pContext);
	...
	// have to be updated by this loop
	...
	// application. Core 1 has no scheduler, so this is a plain delay, not a
	// cooperative sleep (unlike sample/49-elmoplatinum, which runs on core 0).
	CLogger::Get ()->Write (FromElmo, LogNotice,
				"Waiting %u us before requesting OPERATIONAL",
				SAFE_OP_TO_OPERATIONAL_DELAY_US);

	CTimer::SimpleusDelay (SAFE_OP_TO_OPERATIONAL_DELAY_US);
	...
	// require. This request is unconditional, see the comment above
	// Configure() — arming/disarming is a drive-level (CiA 402) concept.
...
boolean CElmoPlatinumCore1Master::ConfigureDrive (void)
{
	assert (m_pContext != 0);
...
//
// Cyclic process data exchange and the CiA 402 enable/disable sequence.
//
// Unlike sample/49-elmoplatinum, every branch that would enable the drive
// checks m_bRunning (the armed/disarmed intent, set via Start/Stop commands
// from the ZeroMQ client on core 0) first. There is no autonomous motion:
// the target velocity is always written as 0.
//

void CElmoPlatinumCore1Master::CyclicProcess (void)
{
	assert (m_pContext != 0);
	...
	while (m_bConfigured)
	{
		...
		else
		{
			...
			// decode the statusword and drive the CiA 402 enable/disable
			// sequence, gated by m_bRunning (armed/disarmed)
			TElmoInputs Inputs;
			...
			case CiA402FaultReactionActive:
				// a fault must be clearable regardless of the armed state
				m_usControlWord = CW_FAULT_RESET;
				break;

			case CiA402NotReadyToSwitchOn:
			case CiA402SwitchOnDisabled:
				// transition into Ready to switch on, a prerequisite for
				// arming, not the armed state itself
				m_usControlWord = CW_SHUTDOWN;
				break;

			case CiA402ReadyToSwitchOn:
				// disarmed: CW_SHUTDOWN is a harmless no-op self-transition
				// here, keeping the drive parked in this state
				m_usControlWord = m_bRunning ? CW_SWITCH_ON : CW_SHUTDOWN;
				break;

			case CiA402SwitchedOn:
				m_usControlWord = m_bRunning ? CW_ENABLE_OPERATION : CW_SHUTDOWN;
				break;

			case CiA402OperationEnabled:
				// disarmed: CW_SHUTDOWN is DS402 "Shutdown", the drive lands
				// back in Ready to switch on (power stage disabled) - this
				// is the actual disarm path once armed. This is NOT Quick
				// Stop (which needs objects 0x605A/0x6085 and a different
				// controlword bit pattern, not implemented here).
				m_usControlWord = m_bRunning ? CW_ENABLE_OPERATION : CW_SHUTDOWN;
				break;

			case CiA402QuickStopActive:
				// only ever reached by drive-internal/external logic, never
				// commanded by this master
				m_usControlWord = CW_SHUTDOWN;
				break;
			...
			// no motion-command path exists in this sample
			m_nTargetVelocity = 0;

			if (PreviousState != m_DriveState)
			{
				CLogger::Get ()->Write (FromElmo, LogNotice,
					"Drive state: %s (statusword 0x%04X, %s)",
					GetDriveStateString (m_DriveState),
					(unsigned) m_usStatusWord,
					m_bRunning ? "armed" : "disarmed");
			}

			TElmoOutputs Outputs;
			Outputs.ControlWord = m_usControlWord;
			Outputs.TargetVelocity = m_nTargetVelocity;
...
TCiA402State CElmoPlatinumCore1Master::DecodeStatusWord (u16 usStatusWord)
{
...
void CElmoPlatinumCore1Master::ReadSlaveInfo (void)
{
	assert (m_pContext != 0);
...
void CElmoPlatinumCore1Master::ReadSlaveSDOString (u16 usIndex, char *pString, size_t nSize)
{
	assert (pString != 0);
	assert (nSize > 0);
...
void CElmoPlatinumCore1Master::UpdateStatus (void)
{
	assert (m_pStatus != 0);
...
void CElmoPlatinumCore1Master::GetStatus (TElmoPlatinumStatus *pStatus)
{
	assert (pStatus != 0);
	assert (m_pStatus != 0);
...
void CElmoPlatinumCore1Master::SubmitCommand (TElmoCommand Command)
{
...
TElmoCommand CElmoPlatinumCore1Master::FetchAndClearCommand (void)
{
	m_CommandSpinLock.Acquire ();

	TElmoCommand Command = m_Command;
	m_Command = ElmoCommandNone;
...
void CElmoPlatinumCore1Master::HandleCommand (void)
{
	TElmoCommand Command = FetchAndClearCommand ();

	switch (Command)
	{
	case ElmoCommandStart:
		if (!m_bRunning)
		{
			m_bRunning = TRUE;

			CLogger::Get ()->Write (FromElmo, LogNotice,
						"Start requested, arming the drive");
		}
		break;
	...
	case ElmoCommandStop:
		if (m_bRunning)
		{
			...
			CLogger::Get ()->Write (FromElmo, LogNotice,
						"Stop requested, disarming the drive");
		}
		break;

	case ElmoCommandNone:
	default:
		break;
	}
}

void CElmoPlatinumCore1Master::SetError (const char *pMessage)
{
	assert (pMessage != 0);
...
void CElmoPlatinumCore1Master::ClearError (void)
{
	m_LastError = "";
	m_LoggedError = "";
}

void CElmoPlatinumCore1Master::SetState (TElmoMasterState State)
{
	m_State = State;
}

const char *CElmoPlatinumCore1Master::GetMasterStateString (TElmoMasterState State)
{
...
const char *CElmoPlatinumCore1Master::GetDriveStateString (TCiA402State State)
{
	static const char *StateString[] =
	{
